...
export function isUserAdmin() {
    ...
    if (typeof role === "string" && role.toLowerCase() === "admin" || role === "SuperAdmin") return true;
    const roles = payload.roles || payload?.user?.roles;
    if (Array.isArray(roles) && roles.some((r: any) => String(r).toLowerCase() === "admin")|| role === "superadmin") return true;
    return false;
}
