...
const SubmitTaskUI: React.FC<SubmitTaskProps> = ({ taskData }) => {
    ...
    if (type === "checkbox") {
      ...
      if (name === "volume") {
        ...
        if (/^n\/?a?$/i.test(trimmed)) {
          setSubmission((prev) => ({ ...prev, volume: value.toUpperCase() }));
          return;
        }
        if (/^n\/?a$/i.test(trimmed)) {
          setSubmission((prev) => ({ ...prev, volume: "N/A" }));
          return;
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("totalCredit", submission.totalCredit || "");
      ...
      // formData.append("security", submission.security || "");
      // formData.append("endpoint", submission.endpoint || "");
      // formData.append("volume", submission.volume || "");
      // formData.append("proxy", submission.proxy || "");
      // formData.append("credit", submission.credit || "");
  ...
  useEffect(() => {
    const entries = Object.values(submission.proxyDetailes || []);
    const totalCredit = entries
      .filter((p: any) => p.proxy !== "N/A")
      .reduce((sum, p: any) => sum + (p.credit || 0), 0);
    const totalRequest = entries.length;
    setSubmission(prev => ({ ...prev, totalCredit, totalRequest }));
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            {sections.map((section) => (
              ...
              <div key={section.id} className="bg-white border border-blue-200 rounded-2xl shadow-md shadow-blue-100 p-6">
                ...
                {section.id === 2 && submission.feasible !== false && (
                  ...
                  <div className="md:col-span-2 mt-4">
                    <div className="flex justify-end items-center mb-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const nextIndex = Object.keys(submission.proxyDetailes || {}).length;
                          setSubmission(prev => ({
                            ...prev,
                            proxyDetailes: {
                              ...prev.proxyDetailes,
                          ...
                          setErrors(prev => ({ ...prev, proxyDetailes: "" }));
                        }}
                    ...
                    <table className="min-w-full border rounded-md overflow-hidden text-sm">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="px-3 py-2 border">Security/Captcha</th>
                      ...
                      <tbody>
                        {Object.entries(submission.proxyDetailes || {}).map(([key, p]: any) => (
                          <tr key={key} className="even:bg-gray-50">
                            ...
                            <td className="px-2 py-1 border">
                              <input
                                type="text"
                                placeholder="Ex:- 45000 / 4K / 3M / 2B or N/A"
                                value={p.volume}
                                onChange={(e) => {
                                  let raw = e.target.value.toUpperCase();
                                  const errorKey = `proxy_volume_${key}`;
                                  ...
                                  if (/^[0-9KMB\.]*$/.test(raw)) {
                                    ...
                                    toast.error("Invalid character! Only numbers, dot, K, M, B, or N/A allowed");
                                  }
                                }}
                                ...
                                onBlur={() => {
                                  ...
                                  if (!isValidVolume(value)) {
                                    ...
                                    toast.error("Invalid volume format. Use 45000 / 4K / 3M / 2B or N/A");
                                    return;
                                  }
